...
class PicarroFrame(tk.Frame):
    def __init__(self, master, reader, conf, *args, **kwargs):
        ...
        self.conf = conf
        self.parList = conf["plotPars"]
        self.plotMinutes = conf["plotMinutes"]
        self.evalSeconds = conf["evalSeconds"]
        self.critParTable = conf["critPars"]
    ...
    def on_resize(self,event):
        ...
        for i in range(len(self.parList)):
            self.parCanvas[i].on_resize(width-100,height/len(self.parList)-5)           
...
if __name__ == "__main__":
    ...
    import configLoader
    confDat = configLoader.load_confDict("../config/picarro.cfg")
    ...
    pf = PicarroFrame(root, lfr, confDat)
