...
class MinipulsFrame(tk.Frame):
    def __init__(self, master, fc, flowConfigFile="../config/minipuls.cfg", *args, **kwargs):
        super(MinipulsFrame,self).__init__(master,*args, **kwargs)
...
if __name__ == "__main__":
    ...
    calibration = configLoader.load_confDict("../config/minipuls.cfg")["calibration"]
    ...
    root = tk.Tk()
    root.title("MinipulsController")
    root.geometry("%dx%d+%d+%d"%(1280,480,1,0))    
    gui = MinipulsFrame(root, fc, flowConfigFile="../config/minipuls.cfg", relief=tk.RAISED)
